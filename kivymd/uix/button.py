...
<BaseButton>
    canvas:
        ...
        Color:
            rgba:
                [1,1,1,1] if not self.disabled else \
        ...
        RoundedRectangle:
            ...
            pos: self.pos
            radius: [root._radius, ]
            texture: Gradient.vertical(root.gradient_light, root.gradient_dark if root.gradient_dark else root.gradient_light) 

    lbl_txt: lbl_txt
    size_hint: None, None
    height: dp(30) + lbl_txt.texture_size[1]
...
class BaseButton(ThemableBehavior, ButtonBehavior, AnchorLayout):
    ...
    text = StringProperty(" ")
    gradient_light = ColorProperty()
    gradient_dark = ColorProperty()
    """
    Button text.
    ...
    def __init__(self, **kwargs):
        ...
        self.gradient_light = self.md_bg_color
        if not self.text_color:
            self.text_color = self.theme_cls.text_color

    def on_gradient_light(self, *args):
        if self.gradient_dark:
            self.gradient_dark = self.gradient_light
    ...
    def on_disabled(self, instance, value):
        ...
        # Sets last current button background color.
            ...
            self.md_bg_color = (
                self._md_bg_color if self._md_bg_color else self.theme_cls.primary_color
            )
...
class BasePressedButton(BaseButton):
    ...
    def on_touch_up(self, touch):
        if (
            ...
            Animation(duration=0.05, md_bg_color=self.current_md_bg_color).start(self)
...
class BaseElevationButton(CommonElevationBehavior, BaseButton):
    ...
    _elevation_raised = NumericProperty(10)
    _anim_raised = None
...
class MDIconButton(BaseRoundButton, BasePressedButton):
    ...
    def set_size(self, interval):
        ...
        self.width = "48dp" if not self.user_font_size else dp(self.user_font_size)
        self.height = "48dp" if not self.user_font_size else dp(self.user_font_size)
...
class MDFloatingActionButton(
    ...
    def set_size(self, interval):
        self.width = "56dp" if not self.user_font_size else dp(self.user_font_size)
        self.height = "56dp" if not self.user_font_size else dp(self.user_font_size)
